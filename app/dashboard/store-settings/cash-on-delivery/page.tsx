...
import { CashOnDeliveryForm } from "../_components/cash-on-delivery-form"
...
export default function CashOnDeliveryPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Cash on Delivery Settings</h1>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Configure Cash on Delivery</CardTitle>
          <CardDescription>Enable or disable cash on delivery payment option</CardDescription>
        </CardHeader>
        <CardContent>
          {storeConfig && userId && <CashOnDeliveryForm initialData={storeConfig} userId={userId} />}
        </CardContent>
      </Card>
    </div>
  )
