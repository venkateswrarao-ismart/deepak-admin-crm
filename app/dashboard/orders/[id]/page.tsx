...
//               <div className="p-1 border-r border-black"></div>
//               <div className="p-1 border-r border-black"></div>
//               <div className="p-1 text-right pr-2 text-red-600">-{formatCurrency(order.total_amount)}</div>
//             </div>
//           )}
...
export default function OrderDetailsPage({
  ...
  const handleEditOrderItem = async (item: any) => {
    const newQty = parseInt(editQty.toString())
    const newPrice = parseFloat(editPrice)
  ...
  const calculateRentalPeriod = () => {
    ...
    if (order.rental_hours) {
      return `${order.rental_hours} hours`
    } else if (order.rental_days) {
      return `${order.rental_days} day${order.rental_days > 1 ? 's' : ''}`
  ...
  const getRentalDates = () => {
    if (!order) return null
    
    if (order.rental_start_datetime && order.rental_end_datetime) {
      return {
        start: order.rental_start_datetime,
        end: order.rental_end_datetime
      }
    } else if (order.start_date && order.end_date) {
      return {
  ...
  const rentalDates = getRentalDates()
    ...
    <div className="space-y-6">
      ...
      <div className="border border-black print-section" style={{
        ...
        <div className="flex w-full">
          ...
          <div className="w-1/2 grid grid-cols-2">
            {[
              ...
              { label: "Buyer's Order No.", value: order.id },
              { label: "Dated", value: format(new Date(order.created_at), "dd-MMM-yyyy"), bold: true },
              { label: "Rental Period", value: rentalPeriod || "N/A", bold: true, colSpan: 2 },
              { label: "Rental Start", value: rentalDates?.start ? format(new Date(rentalDates.start), "dd-MMM-yyyy hh:mm a") : "N/A", bold: true },
              { label: "Rental End", value: rentalDates?.end ? format(new Date(rentalDates.end), "dd-MMM-yyyy hh:mm a") : "N/A", bold: true },
              { label: "Destination", value: "" },
              { label: "Local Deliveries", value: "" },
              { label: "Vehicle Number", value: "" },
              {
              ...
              <div
                key={index}
                className={`p-1 ${index % 2 === 0 ? "border-r border-black" : ""} ${index < 20 ? "border-b border-black" : ""}`}
                style={{ gridColumn: item.colSpan ? "span 2" : "span 1" }}
              >
                <div className="text-xs">{item.label}</div>
                <div className={`${item.bold ? "font-bold" : ""}`}>{item.value}</div>
        ...
        <div className="border border-black mt-1 text-xs">
          ...
          {/* Subtotal Row */}
        {/* Subtotal Row - Show calculation for rental period */}
<div className="grid grid-cols-[34px_2fr_1fr_1fr_1fr_1fr_50px_1fr] font-bold">
  <div className="p-1 border-r border-black"></div>
  <div className="p-1 border-r border-black">Subtotal (Daily Rate)</div>
  <div className="p-1 border-r border-black"></div>
  <div className="p-1 border-r border-black"></div>
  <div className="p-1 border-r border-black"></div>
  <div className="p-1 border-r border-black"></div>
...
{/* Rental Period Calculation Row */}
<div className="grid grid-cols-[34px_2fr_1fr_1fr_1fr_1fr_50px_1fr]">
  <div className="p-1 border-r border-black"></div>
  <div className="p-1 border-r border-black">
    × {order.rental_days || 1} day{order.rental_days > 1 ? 's' : ''}
    {order.rental_hours ? ` (${order.rental_hours} hours)` : ''}
  </div>
  <div className="p-1 border-r border-black"></div>
  <div className="p-1 border-r border-black"></div>
  <div className="p-1 border-r border-black"></div>
  <div className="p-1 border-r border-black"></div>
  <div className="p-1 border-r border-black"></div>
  <div className="p-1 text-right pr-2 font-semibold">
    = {formatCurrency(order.total_amount)}
      ...
      {/* Other Interactive Components */}
      <div className="space-y-6 print-hide">
        {/* Rental Period Information Card */}
        {rentalPeriod && (
          <Card className="p-4">
            <CardHeader>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="font-semibold">{rentalPeriod}</div>
                </div>
                {rentalDates?.start && (
                  <div>
                    <Label>Rental Start Date</Label>
                    <div className="font-semibold">
                      {format(new Date(rentalDates.start), "dd-MMM-yyyy hh:mm a")}
                    </div>
                  </div>
                )}
                {rentalDates?.end && (
                  <div>
                    ...
                    <div className="font-semibold">
                      {format(new Date(rentalDates.end), "dd-MMM-yyyy hh:mm a")}
                    </div>
                  </div>
                )}
                <div>
